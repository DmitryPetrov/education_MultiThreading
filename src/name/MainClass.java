...
public class MainClass {
    public static void main(String[] args) {
        ...
        String text = "In a hole in the ground there lived a hobbit. Not a nasty, dirty, wet hole,"
                + "filled with the ends of worms and an oozy smell, nor yet a dry, bare, sandy"
                + "hole with nothing in it to sit down on or to eat: it was a hobbit-hole, and that"
                + "means comfort."
                ...
                + "and lots of pegs for hats and coats Ч the hobbit was fond of visitors."
                + "The tunnel wound on and on, going fairly but not quite straight into the side"
                + "of the hill Ч The Hill, as all the people for many miles round called it Ч and"
                + "many little round doors opened out of it, first on one side and then on another."
                + "No going upstairs for the hobbit: bedrooms, bathrooms, cellars, pantries (lots"
                + "of these), wardrobes (he had whole rooms devoted to clothes), kitchens,"
                ...
                + "meadows beyond, sloping down to the river.This hobbit was a very well-to-do "
                ...
                + "lost the neighboursТ respect, but he gained Ч well, you will see whether he"
                + "gained anything in the end.";

        FrameContainer outputContainer = new StringParser().parse(text, " ");
        FrameContainer inputContainer = new FrameContainer();

        Buffer buffer = new Buffer();

        String outputContString = outputContainer.toString();
        outputContainer.print();
        
        PoolOfThreads pool = new PoolOfThreads(outputContainer, inputContainer,
                buffer);

        try {
            pool.createAllThreads(3);
            pool.startAllThreads();
            pool.joinWriters();
            if (outputContainer.isEmpty()) {
                buffer.setEndOfTransfer();
                System.out.println("[ѕередача завершена]");
            }

            pool.joinReaders();
            System.out.println("[ѕрием завершен]");
        } catch (InterruptedException e) {
            System.out.println("Interrupt main thread" + e);
        }
        ...
        if (inputContainer.equals(outputContString)) {
            System.out.println("[ѕередача прошла успешнео]");
        } else {
            System.out.println("[ѕередача прошла неудачно]");
        }
